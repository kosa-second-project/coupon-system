...
#### 4) 테스트 데이터 리셋 시 가상 유저 무덤 현상(DB 비대화) 및 선별 정리(Garbage Collection) 도입
* **문제 상황**: k6 부하 테스트 실행 시마다 수만 명의 고유한 임시 가상 유저(`test_user_...`)가 생성됩니다. 테스트 데이터를 리셋할 때 이 유저들을 그대로 방치하면 DB 테이블이 불필요하게 비대해지며(Bloat), 반대로 가상 유저와 이력을 무조건 벌크 삭제하면 **다른 쿠폰 테스트에 참가한 가상 유저 데이터까지 동반 유실되거나 외래키(FK) 참조 제약 위반 에러**가 발생하는 딜레마가 있었습니다.
* **해결 방안**: 데이터베이스 구조를 변경하지 않고 기존 릴레이션을 이용하는 **선별적 정리(Garbage Collection) 알고리즘**을 도입했습니다.
  1. **정밀 타격 삭제**: 현재 리셋 대상인 쿠폰 ID에 연결된 가상 사용자들의 발급 이력(`CouponIssue`)만 먼저 안전하게 조준 삭제합니다.
  2. **잔여 참조 추적**: 삭제 직후 전체 가상 사용자 ID 중에서 **여전히 다른 쿠폰의 발급 이력(참조)이 남아있는 활성 유저 ID** 목록을 역으로 조회합니다.
  3. **가비지 정리**: 원래 리셋 대상이던 가상 사용자 중, 타 쿠폰 참조 목록에 포함되지 않아 **더 이상 아무런 발급 이력이 남지 않은 가상 사용자만 선별**하여 유저 테이블에서 안전하게 벌크 삭제합니다.
  - 이 방식을 통해 동시성 테스트의 중복성 한계 제약을 막기 위해 동적 가상 유저 생성을 지원하면서도, 테스트 이후 DB 잔여 공간 낭비와 참조 제약 충돌을 완벽히 해결했습니다.
